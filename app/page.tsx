...
type UiIconName = "home" | "search" | "notifications" | "messages" | "bookmark" | "profile" | "settings" | "sun" | "moon" | "arrow-left" | "arrow-right" | "close" | "more" | "media" | "location" | "poll" | "mood" | "send" | "pin" | "quote" | "spark";
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <MobileNav view={view} setView={setView} unread={3 - readNotifications.length} onMore={() => setMobileMoreOpen(true)} />
    ...
    {mobileMoreOpen && <MobileMoreMenu onClose={() => setMobileMoreOpen(false)} onProfile={openOwnProfile} onSettings={() => { setMobileMoreOpen(false); setView("settings"); }} />}
...
function MobileNav({ view, setView, unread, onMore }: { view: View; setView: (view: View) => void; unread: number; onMore: () => void }) { return <nav className="bottom-nav" aria-label="Mobil gezinme">{navigation.slice(0, 5).map((item) => <button key={item.id} className={view === item.id ? "is-active" : ""} onClick={() => setView(item.id)} aria-label={item.label}><UiIcon name={item.icon} />{item.id === "notifications" && unread > 0 && <b />}</button>)}<button className={view === "profile" || view === "settings" ? "is-active" : ""} onClick={onMore} aria-label="Daha Fazla"><UiIcon name="more" /></button></nav>; }
function MobileMoreMenu({ onClose, onProfile, onSettings }: { onClose: () => void; onProfile: () => void; onSettings: () => void }) { const dialogRef = useDialogFocus(onClose); return <div className="modal-backdrop"><section ref={dialogRef} className="more-menu mobile-more-menu" role="dialog" aria-modal="true" aria-label="Daha fazla gezinme" tabIndex={-1}><button onClick={onProfile}><UiIcon name="profile" /> Profil</button><button onClick={onSettings}><UiIcon name="settings" /> Ayarlar</button><button onClick={onClose}><UiIcon name="close" /> Vazgeç</button></section></div>; }
