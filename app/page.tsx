...
function Profile(props: FeedProps & { profile: Profile; isOwn: boolean; points: number; gamification: Gamification; onEdit: () => void; onSettings: () => void; onMessage: (profile: Profile) => void; onPoints: () => void }) {
  ...
  const profileBadge = props.isOwn ? <button className="points-summary" onClick={props.onPoints} aria-label={props.points + " puan. Rozet ve kilometre taşlarını gör."}><span className={"points-badge " + badge.tone} aria-hidden="true">{badge.mark}</span><small>{props.points} puan</small></button> : null;
