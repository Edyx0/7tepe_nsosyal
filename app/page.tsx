...
type Post = {
  ...
  image?: string; imageAlt?: string; repostedBy?: string; repostQuote?: string;
};
...
export default function Home() {
  ...
  function publishGuidedContribution(body: string, replyTarget: Post) {
    ...
    setDraftPosts((current) => [post, ...current]); awardTask("deep-contribution", 20); setNotice("Yanıtın konuşmaya eklendi; +20 NSosyal puanı kazandın."); setView("feed");
  }
...
<Sidebar view={view} setView={setView} profile={profile} unread={3 - readNotifications.length} onOwnProfile={openOwnProfile} onCompose={openComposer} onSearch={openSearch} />
    <section className="timeline" aria-label={title}>
       ...
       <div id="main-content" className="content-area">
        ...
        {view === "feed" && <><div className="feed-tabs" role="tablist" aria-label="Ana Sayfa seçimi"><button role="tab" aria-selected={feedMode === "for-you"} className={feedMode === "for-you" ? "selected" : ""} onClick={() => setFeedMode("for-you")}>Sana Özel</button><button role="tab" aria-selected={feedMode === "following"} className={feedMode === "following" ? "selected" : ""} onClick={() => setFeedMode("following")}>Takip Ettiklerin</button></div><Feed {...feedProps} posts={displayedPosts} morePost={morePost} showContributionPrompt={feedMode === "for-you"} /></>}
...
const contributionIntents: ContributionIntent[] = [
  { id: "agree", label: "Neden katıldığını yaz", helper: "Kendi gerekçeni ekle", prompt: "Bu görüşe katılıyorum; çünkü ", icon: "check" },
  { id: "proposal", label: "Bir çözüm öner", helper: "Uygulanabilir bir adım paylaş", prompt: "Bunu ilerletmek için şu adım atılabilir: ", icon: "spark" },
  { id: "source", label: "Kaynak paylaş", helper: "Veri, okuma veya bağlantı ekle", prompt: "Bu noktayı destekleyen kaynak: ", icon: "quote" },
];

function InlineContributionPrompt({ contextPost, onPublish }: { contextPost: Post; onPublish: (body: string, target: Post) => void }) {
  const [intent, setIntent] = useState<ContributionIntent | null>(null);
  const [draft, setDraft] = useState("");
  const [publishedReply, setPublishedReply] = useState<{ body: string; label: string } | null>(null);
  const [status, setStatus] = useState("");
  const selectIntent = (next: ContributionIntent) => { setIntent(next); setStatus(next.label + " seçildi. Yanıtın bu gönderinin altına eklenecek."); };
  const publish = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!intent) { setStatus("Önce yanıtınla ne eklemek istediğini seç."); return; } const replyBody = draft.trim(); if (!replyBody) { setStatus("Yanıtını 1–2 cümleyle yaz."); return; } onPublish(replyBody, contextPost); setPublishedReply({ body: replyBody, label: intent.label }); setDraft(""); setIntent(null); setStatus("Yanıtın gönderildi. +20 NSosyal puanı eklendi."); };
  const contextCopy = contextPost.body.length > 150 ? contextPost.body.slice(0, 147) + "…" : contextPost.body;

  return <article className="inline-contribution" aria-labelledby="inline-contribution-title"><form onSubmit={publish}><header className="inline-contribution-header"><span className="inline-contribution-mark" aria-hidden="true"><UiIcon name="quote" /></span><span><span className="inline-contribution-kicker">Konuşmaya katkı</span><h2 id="inline-contribution-title">Bu gönderiye yanıt ver</h2><p>Birini seç; ardından yanıtın gönderinin altına eklensin.</p></span></header><section className="inline-context" aria-label="Yanıtlanacak gönderi"><span>{contextPost.initials}</span><div><b>{contextPost.name}</b><small>{contextPost.handle}</small><p>“{contextCopy}”</p></div></section>{!publishedReply && <><fieldset className="inline-intents"><legend>Yanıtınla ne eklemek istersin?</legend><div>{contributionIntents.map((item) => <button type="button" key={item.id} className={intent?.id === item.id ? "is-selected" : ""} aria-pressed={intent?.id === item.id} onClick={() => selectIntent(item)}><UiIcon name={item.icon} /><span><b>{item.label}</b><small>{item.helper}</small></span></button>)}</div></fieldset>{intent && <div className="inline-draft"><label htmlFor="inline-contribution-draft">Yanıtını yaz</label><textarea id="inline-contribution-draft" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={intent.prompt} maxLength={280} aria-describedby="inline-contribution-hint" /><footer><small id="inline-contribution-hint">Yanıtın gönderinin altında görünecek · {draft.length}/280</small><button className="inline-publish" type="submit" disabled={!draft.trim()}>Yanıtı gönder</button></footer></div>}</>}{publishedReply && <section className="inline-published-reply" aria-label="Gönderiye eklediğin yanıt"><span className="inline-reply-rail" aria-hidden="true" /><div><p><b>Gönderiye eklediğin yanıt</b><small>{publishedReply.label} · Bağlantılı yorum</small></p><blockquote>{publishedReply.body}</blockquote></div></section>}<p className="inline-contribution-status" role="status" aria-live="polite">{status}</p></form></article>;
}
...
function PostCard({ post, actions, pinned, following, points, onAction, onReply, onDetail, onProfile, onShare, onMore, onMedia, onFollow, threaded = false }: FeedProps & { post: Post; threaded?: boolean }) {
  ...
  return <article className={"post " + (post.replies > 0 || threaded ? "has-thread" : "")}>
