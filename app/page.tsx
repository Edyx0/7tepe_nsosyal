...
type UiIconName = "home" | "search" | "menu" | "notifications" | "messages" | "bookmark" | "profile" | "settings" | "sun" | "moon" | "arrow-left" | "arrow-right" | "close" | "more" | "media" | "location" | "poll" | "mood" | "send" | "pin" | "quote" | "spark";
...
function isValidStored(key: string, value: unknown): boolean {
  ...
  if (key === "nsosyal-messages") return Array.isArray(value) && value.every((item) => !!item && typeof item === "object" && typeof (item as Message).id === "string" && ((item as Message).conversationId === "team" || (item as Message).conversationId === "bora") && ((item as Message).from === "me" || (item as Message).from === "them") && typeof (item as Message).body === "string");
  return true;
}
...
export default function Home() {
  ...
  const openOwnProfile = () => { setProfilePost(null); setMobileMoreOpen(false); setView("profile"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const openSearch = () => { setView("explore"); window.setTimeout(() => document.querySelector<HTMLInputElement>(".explore-search input")?.focus(), 80); };
  ...
  function createPost(event: FormEvent<HTMLFormElement>) {
    ...
    const post: Post = { id: "yerel-" + Date.now(), name: profile.name, handle: profile.handle, time: "şimdi", body: replyingTo ? replyingTo.handle + " " + body : body, initials: profile.initials, tone: "ink", replies: 0, reposts: 0, likes: 0, audience: "following", replyTo: replyingTo?.name, replyToId: replyingTo?.id, attachment: composerMedia ? "signal" : undefined, own: true };
    setDraftPosts((current) => [post, ...current]); setComposerText(""); setComposerMedia(false); setReplyingTo(null); setNotice("Gönderi Ana Sayfa'ya eklendi."); setView("feed");
  }
  ...
  return <main className="app-shell">
    ...
    <MobileNav view={view} setView={setView} unread={3 - readNotifications.length} profile={profile} onMore={() => setMobileMoreOpen(true)} />
    {shareFeedback && <div className="toast" role="status">{shareFeedback}</div>}
    {compactMenuOpen && <CompactDesktopMenu view={view} setView={setView} theme={theme} setTheme={setTheme} profile={profile} onClose={() => setCompactMenuOpen(false)} onOwnProfile={openOwnProfile} />}
...
function MobileNav({ view, setView, unread, profile, onMore }: { view: View; setView: (view: View) => void; unread: number; profile: Profile; onMore: () => void }) { return <nav className="bottom-nav" aria-label="Mobil gezinme">{navigation.slice(0, 4).map((item) => <button key={item.id} className={view === item.id ? "is-active" : ""} onClick={() => setView(item.id)} aria-current={view === item.id ? "page" : undefined} aria-label={item.label}><UiIcon name={item.icon} />{item.id === "notifications" && unread > 0 && <b />}</button>)}<button className={view === "profile" || view === "settings" ? "is-active" : ""} onClick={onMore} aria-current={view === "profile" || view === "settings" ? "page" : undefined} aria-label="Profil ve daha fazlası"><Avatar initials={profile.initials} tone="ink" /></button></nav>; }
...
function CompactDesktopMenu({ view, setView, theme, setTheme, profile, onClose, onOwnProfile }: { view: View; setView: (view: View) => void; theme: Theme; setTheme: (theme: Theme) => void; profile: Profile; onClose: () => void; onOwnProfile: () => void }) {
  const dialogRef = useDialogFocus(onClose);
  const navigate = (next: View) => { setView(next); onClose(); };
...
function Messages({ messages, setMessages, profile }: { messages: Message[]; setMessages: (next: Message[]) => void; profile: Profile }) {
  const [open, setOpen] = useState<"team" | "bora">("team"); const [draft, setDraft] = useState("");
