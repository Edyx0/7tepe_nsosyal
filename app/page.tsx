...
const navigation: { id: View; label: string; icon: string }[] = [
  { id: "feed", label: "Akış", icon: "⌂" }, { id: "explore", label: "Keşfet", icon: "⌕" },
...
export default function Home() {
  ...
  useEffect(() => {
    const restore = window.setTimeout(() => {
      setTheme(readStored("yanki-theme", "light"));
      setActions(readStored("yanki-actions", { likes: [], reposts: [], bookmarks: [] }));
      setMods(readStored("yanki-mods", { deleted: [], pinned: null }));
      setDraftPosts(readStored("yanki-drafts", []));
      setProfile(readStored("yanki-profile", demoProfile));
      setFollowing(readStored("yanki-following", ["@bariskoral"]));
      setMessages(readStored("yanki-messages", [{ id: "welcome", from: "them", body: "Bağlam özeti, konuşmaya yetişmene yardımcı olur. Her zaman kaynaklara göz atmayı unutma." }]));
      setReadNotifications(readStored("yanki-read-notifications", []));
      restoredLocalState.current = true;
    }, 0);
    return () => window.clearTimeout(restore);
  }, []);
  useEffect(() => { document.documentElement.dataset.theme = theme; if (restoredLocalState.current) window.localStorage.setItem("yanki-theme", JSON.stringify(theme)); }, [theme]);
  useEffect(() => { if (restoredLocalState.current) window.localStorage.setItem("yanki-actions", JSON.stringify(actions)); }, [actions]);
  useEffect(() => { if (restoredLocalState.current) window.localStorage.setItem("yanki-mods", JSON.stringify(mods)); }, [mods]);
  useEffect(() => { if (restoredLocalState.current) window.localStorage.setItem("yanki-drafts", JSON.stringify(draftPosts)); }, [draftPosts]);
  useEffect(() => { if (restoredLocalState.current) window.localStorage.setItem("yanki-profile", JSON.stringify(profile)); }, [profile]);
  useEffect(() => { if (restoredLocalState.current) window.localStorage.setItem("yanki-following", JSON.stringify(following)); }, [following]);
  useEffect(() => { if (restoredLocalState.current) window.localStorage.setItem("yanki-messages", JSON.stringify(messages)); }, [messages]);
  useEffect(() => { if (restoredLocalState.current) window.localStorage.setItem("yanki-read-notifications", JSON.stringify(readNotifications)); }, [readNotifications]);
  ...
  async function sharePost(post: Post) {
    try { await navigator.clipboard.writeText(post.name + ": " + post.body); setShareFeedback("Yankı metni panoya kopyalandı."); } catch { setShareFeedback("Paylaşmaya hazır: metni seçip kopyalayabilirsin."); }
  ...
  function createPost(event: FormEvent<HTMLFormElement>) {
    ...
    setDraftPosts((current) => [post, ...current]); setComposerText(""); setComposerMedia(false); setReplyingTo(null); setNotice("Yankın akışa eklendi."); setView("feed");
  }
  ...
  function deleteOwnPost(post: Post) { setMods((current) => ({ ...current, deleted: removeItem(current.deleted, post.id), pinned: current.pinned === post.id ? null : current.pinned })); setMorePost(null); setSelectedPost(null); setNotice("Yankın bu cihazdaki akıştan kaldırıldı."); setView("feed"); }
  function togglePin(post: Post) { setMods((current) => ({ ...current, pinned: togglePinned(current.pinned, post.id) })); setMorePost(null); setNotice(mods.pinned === post.id ? "Yankı sabitlemeden kaldırıldı." : "Yankı profiline sabitlendi."); }
  function resetDemo() {
    ["yanki-actions", "yanki-mods", "yanki-drafts", "yanki-profile", "yanki-following", "yanki-messages", "yanki-read-notifications"].forEach((key) => window.localStorage.removeItem(key));
    setActions({ likes: [], reposts: [], bookmarks: [] }); setMods({ deleted: [], pinned: null }); setDraftPosts([]); setProfile(demoProfile); setFollowing(["@bariskoral"]); setMessages([{ id: "welcome", from: "them", body: "Bağlam özeti, konuşmaya yetişmene yardımcı olur. Her zaman kaynaklara göz atmayı unutma." }]); setReadNotifications([]); setNotice("Demo başlangıç durumuna döndü."); setView("feed");
  }
  ...
  const visibleProfile: Profile = profilePost && !profilePost.own ? { name: profilePost.name, handle: profilePost.handle, initials: profilePost.initials, bio: "Yankı’daki açık konuşmalara katılıyor. Bu profil, çevrimdışı demo verisiyle gösteriliyor.", location: "Türkiye" } : profile;
  ...
  return <main className="app-shell">
    ...
    <section className="timeline" aria-label={title}>
      ...
      <div id="main-content" className="content-area">
        ...
        {view === "bookmarks" && <><div className="view-intro"><p>Kaydettiğin konuşmalar burada, bu cihazda kalır.</p></div>{displayedPosts.length ? <Feed {...feedProps} posts={displayedPosts} morePost={morePost} /> : <EmptyState title="Henüz yer imi yok" copy="İlginç bir yankıyı sonra dönmek için kaydedebilirsin." />}</>}
    ...
    <aside className="right-rail" aria-label="Gündem ve öneriler"><div className="search-field"><span aria-hidden="true">⌕</span><input aria-label="Yankı ara" placeholder="Yankı ara" value={search} onChange={(event) => { setSearch(event.target.value); setView("explore"); }} /></div><TrendPanel onSelect={(topic) => { setSearch(topic); setView("explore"); }} /><WhoToFollow following={following} onFollow={toggleFollow} /><p className="legal">Koşullar · Gizlilik · Çerezler<br />Yankı 2026</p></aside>
...
function Composer({ text, setText, media, onTool, replyingTo, clearReply, onSubmit }: { text: string; setText: (value: string) => void; media: boolean; onTool: (kind: "media" | "location" | "poll" | "mood") => void; replyingTo: Post | null; clearReply: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { return <form className="composer" onSubmit={onSubmit}><Avatar initials="DN" tone="ink" /><div className="composer-body">{replyingTo && <div className="replying">Şuna yanıtlıyorsun: <b>{replyingTo.name}</b><button type="button" onClick={clearReply} aria-label="Yanıtı iptal et">×</button></div>}<label className="sr-only" htmlFor="compose-input">Yeni yankı yaz</label><textarea id="compose-input" value={text} onChange={(event) => setText(event.target.value.slice(0, 280))} placeholder={replyingTo ? "Yanıtını yaz" : "Neler yankılanıyor?"} rows={2} />{media && <div className="composer-media"><span className="signal-art"><i /><i /><i /></span><span><b>Görsel taslağı</b><small>Yankı ile birlikte paylaşılacak</small></span><button type="button" onClick={() => onTool("media")} aria-label="Görsel taslağını kaldır">×</button></div>}<div className="compose-footer"><div className="compose-tools" aria-label="Yankı ekleri"><button type="button" onClick={() => onTool("media")} aria-label="Görsel taslağı ekle">▧</button><button type="button" onClick={() => onTool("location")} aria-label="Konum ekle">⌖</button><button type="button" onClick={() => onTool("poll")} aria-label="Anket taslağı ekle">▤</button><button type="button" onClick={() => onTool("mood")} aria-label="Duygu ekle">☺</button></div><span className={text.length > 250 ? "limit near" : "limit"}>{280 - text.length}</span><button className="primary-button small-button" type="submit">Yankıla</button></div></div></form>; }
...
function PostCard({ post, actions, pinned, following, onAction, onReply, onDetail, onProfile, onQuote, onShare, onMore, onMedia, onFollow }: FeedProps & { post: Post }) {
  ...
  return <article className="post">{pinned === post.id && <p className="pin-status">⌖ Profile sabitlendi</p>}<button className="avatar-button" onClick={(event) => runExclusive(event, () => onProfile(post))} aria-label={post.name + " profilini aç"}><Avatar initials={post.initials} tone={post.tone} /></button><div className="post-content">{post.replyTo && <p className="reply-context">{post.replyTo} adlı kişiye yanıt olarak</p>}<div className="post-meta"><button className="name" onClick={(event) => runExclusive(event, () => onProfile(post))}>{post.name}</button><span>{post.handle}</span><span>·</span><button className="time" onClick={() => onDetail(post)}>{post.time}</button><button className="more" aria-label="Yankı seçeneklerini aç" onClick={(event) => runExclusive(event, () => onMore(post))}>•••</button></div><button className="post-body" onClick={() => onDetail(post)} aria-label="Yankının ayrıntılarını aç">{post.body}</button>{post.attachment === "signal" && <button className="signal-card" onClick={(event) => runExclusive(event, () => onMedia(post))}><span className="signal-art"><i /><i /><i /></span><span><b>Şehir serinliği notları</b><small>Görsel önizlemeyi aç</small></span><em>↗</em></button>}{post.attachment === "note" && <button className="note-card" onClick={(event) => runExclusive(event, () => onMedia(post))}><span>Okuma notu</span><b>Yerel veriyi ortak bir dilde buluşturmak</b><small>Önizlemeyi aç · 4 dk okuma</small></button>}<div className="post-actions" aria-label="Yankı işlemleri"><ActionButton icon="◌" label="Yanıtla" count={post.replies} onClick={() => onReply(post)} /><ActionButton icon="↻" label="Yeniden yankıla" count={post.reposts + Number(reposted)} active={reposted} onClick={() => onAction("reposts", post.id)} onAux={() => onQuote(post)} /><ActionButton icon="♡" label="Beğen" count={post.likes + Number(liked)} active={liked} kind="like" onClick={() => onAction("likes", post.id)} /><ActionButton icon={saved ? "▰" : "▱"} label={saved ? "Yer iminden kaldır" : "Yer imlerine ekle"} active={saved} onClick={() => onAction("bookmarks", post.id)} /><ActionButton icon="⌑" label="Paylaş" onClick={() => onShare(post)} /></div>{!post.own && <button className="post-follow" aria-pressed={following.includes(post.handle)} onClick={(event) => runExclusive(event, () => onFollow(post.handle))}>{following.includes(post.handle) ? "Takiptesin" : "Takip et"}</button>}</div></article>;
}
function ActionButton({ icon, label, count, active, kind, onClick, onAux }: { icon: string; label: string; count?: number; active?: boolean; kind?: "like"; onClick: () => void; onAux?: () => void }) { return <span className={"action-wrap " + (active ? "active " : "") + (kind || "")}><button className="action" aria-label={label} aria-pressed={typeof active === "boolean" ? active : undefined} onClick={(event) => runExclusive(event, onClick)}><i aria-hidden="true">{icon}</i>{typeof count === "number" && <small>{formatNumber(count)}</small>}</button>{onAux && <button className="quote-mini" onClick={(event) => runExclusive(event, onAux)} aria-label="Alıntıla">＋</button>}</span>; }
function ThreadDetail(props: FeedProps & { post: Post; contextOpen: boolean; setContextOpen: (value: boolean) => void }) { const replies = props.post.context ? repliesForThread(seedReplies, props.post) : []; return <div className="thread-view"><PostCard {...props} />{props.post.context && <section className="context-summary" aria-label="Bağlam özeti"><button className="context-toggle" onClick={() => props.setContextOpen(!props.contextOpen)} aria-expanded={props.contextOpen}><span><i aria-hidden="true">✦</i><b>Bağlam Özeti</b><small>Bu konuşmada öne çıkanlar</small></span><span aria-hidden="true">{props.contextOpen ? "⌃" : "⌄"}</span></button>{props.contextOpen && <div className="summary-content"><ul>{contextBullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul><p>AI özeti <span>•</span> hata içerebilir</p></div>}</section>}<div className="thread-reply"><Avatar initials="DN" tone="ink" /><button onClick={() => props.onReply(props.post)}>Yanıtını yaz</button></div>{replies.length ? <Feed {...props} posts={replies} /> : <EmptyState title="Konuşma burada sakin" copy="İlk düşünceni paylaşarak bu yankıyı büyütebilirsin." />}</div>; }
