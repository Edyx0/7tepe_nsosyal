...
function Explore(props: Omit<FeedProps, "posts" | "morePost"> & { search: string; setSearch: (value: string) => void; posts: Post[] }) { const visible = postsForTrend(props.posts, props.search); return <div className="explore-view"><div className="explore-search"><span aria-hidden="true"><UiIcon name="search" /></span><input value={props.search} onChange={(event) => props.setSearch(event.target.value)} placeholder="Kişi ya da konu ara" aria-label="Kişi ya da konu ara" /></div>{props.search ? <><h2>Arama sonuçları</h2>{visible.length ? <div className="search-results"><Feed {...props} posts={visible} morePost={null} /></div> : <EmptyState title="Bir sonuç bulamadık" copy="Başka bir kelimeyle aramayı dene." />}</> : <><h2>Bugün konuşulanlar</h2><TrendPanel full onSelect={props.setSearch} /><section className="editor-pick"><p>Editörün seçimi</p><button onClick={() => props.onDetail(seedPosts[0])}><span className="editor-ripple"><i /><i /></span><span><b>Şehir nasıl serin kalır?</b><small>İklim ve kamusal alan üzerine açık konuşma</small></span><em aria-hidden="true"><UiIcon name="arrow-right" /></em></button></section></>}</div>; }
