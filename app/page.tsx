...
type ContributionIntent = { id: string; label: string; prompt: string };
const contributionIntents: ContributionIntent[] = [
  { id: "agree", label: "Katılıyorum", prompt: "Bu görüşe katılıyorum; çünkü " },
  { id: "proposal", label: "Somut öneri", prompt: "Bunu ilerletmek için şu adım atılabilir: " },
  { id: "source", label: "Kaynak ekle", prompt: "Bu noktayı destekleyen kaynak: " },
];

function InlineContributionPrompt({ onPublish }: { onPublish: (body: string) => void }) {
  const [intent, setIntent] = useState<ContributionIntent | null>(null);
  const [draft, setDraft] = useState("");
  const [status, setStatus] = useState("");
  const selectIntent = (next: ContributionIntent) => { setDraft((current) => !current.trim() || current === intent?.prompt ? next.prompt : current); setIntent(next); setStatus(next.label + " seçildi. Şimdi fikrini kendi cümlenle tamamla."); };
  const publish = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!intent) { setStatus("Önce konuşmaya nasıl katkı vereceğini seç."); return; } if (!draft.trim() || draft.trim() === intent.prompt.trim()) { setStatus("Kısa fikrini kendi cümlenle tamamla."); return; } onPublish(draft); setDraft(""); setIntent(null); setStatus("Katkın yayınlandı. +20 NSosyal puanı eklendi."); };

  return <article className="inline-contribution" aria-labelledby="inline-contribution-title"><form onSubmit={publish}><header className="inline-contribution-header"><span className="inline-contribution-mark" aria-hidden="true"><UiIcon name="spark" /></span><span><h2 id="inline-contribution-title">Katkını derinleştir</h2><p>Bu konuşmayı bir adım ileri taşı.</p></span></header><fieldset className="inline-intents"><legend>Yaklaşımını seç</legend><div>{contributionIntents.map((item) => <button type="button" key={item.id} className={intent?.id === item.id ? "is-selected" : ""} aria-pressed={intent?.id === item.id} onClick={() => selectIntent(item)}>{item.label}</button>)}</div></fieldset>{intent && <div className="inline-draft"><label htmlFor="inline-contribution-draft">{intent.label} · fikrini 1–2 cümlede yaz</label><textarea id="inline-contribution-draft" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={intent.prompt} maxLength={280} aria-describedby="inline-contribution-hint" /><footer><small id="inline-contribution-hint">Kısa, somut ve saygılı kal · {draft.length}/280</small><button className="inline-publish" type="submit" disabled={!draft.trim() || draft.trim() === intent.prompt.trim()}>Katkını yayınla</button></footer></div>}<p className="inline-contribution-status" role="status" aria-live="polite">{status}</p></form></article>;
}
...
type FeedProps = { posts: Post[]; allPosts: Post[]; actions: Actions; pinned: string | null; morePost: Post | null; following: string[]; points: number; onAction: (kind: keyof Actions, id: string) => void; onReply: (post: Post) => void; onDetail: (post: Post) => void; onProfile: (post: Post) => void; onQuote: (post: Post) => void; onShare: (post: Post) => void; onMore: (post: Post | null) => void; onMedia: (post: Post) => void; onDelete: (post: Post) => void; onPin: (post: Post) => void; onFollow: (handle: string) => void; onContribution?: (body: string) => void; showContributionPrompt?: boolean; };
function Feed(props: FeedProps) { return <div className="post-list">{props.posts.map((post, index) => { const displayPost = withLocalReplyCount(post, props.allPosts); return [<PostCard key={post.id} {...props} post={displayPost} />, props.showContributionPrompt && index === 1 && props.onContribution ? <InlineContributionPrompt key="inline-contribution" onPublish={props.onContribution} /> : null]; })}</div>; }
