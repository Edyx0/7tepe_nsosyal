...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="timeline" aria-label={title}>
      <header className="topbar">{view === "detail" ? <button className="back-button" onClick={() => setView("feed")} aria-label="Ana Sayfaya dön">←</button> : <span className="mobile-brand"><BrandMark small /><span className="sr-only">NSosyal</span></span>}<h1>{view === "detail" ? "NSosyal" : title}</h1><button className="top-action" onClick={() => setTheme((current) => current === "light" ? "dark" : "light")} aria-label="Temayı değiştir">{theme === "light" ? "◐" : "☀"}</button></header>
