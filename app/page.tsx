...
function Profile(props: FeedProps & { profile: Profile; isOwn: boolean; onEdit: () => void; onMessage: (profile: Profile) => void }) {
  ...
  const visible = tab === "posts" ? (props.isOwn ? props.posts.filter((post) => !post.replyTo && (post.own || props.actions.reposts.includes(post.id))).map((post) => post.own ? post : { ...post, repostedByMe: true }) : props.posts.filter((post) => belongsToProfile(post) && !post.replyTo)) : tab === "replies" ? props.posts.filter((post) => belongsToProfile(post) && !!post.replyTo) : (props.isOwn ? props.posts.filter((post) => props.actions.likes.includes(post.id)) : []);
  const copy = tab === "posts" ? "Ana Sayfadan yeni bir düşünce başlatabilirsin." : tab === "replies" ? "Bir gönderiye yanıt verdiğinde burada görünür." : "Beğendiğin gönderiler burada görünür.";
