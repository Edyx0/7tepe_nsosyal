...
const notificationIds = ["selin", "veri", "secim"];
const dailyTasks = [
...
const seedPosts: Post[] = [
  ...
  { id: "durak-golgesi", name: "Nisan Aksoy", handle: "@nisanaksoy", time: "10 sa", body: "Durağa iki metrelik gölgelik eklenmiş. Küçük bir şey gibi duruyor ama öğlen orada bekleyen herkes aynı yere toplanmıştı. Yanına su koyarlarsa tamamdır.", initials: "NA", tone: "gold", replies: 17, reposts: 21, likes: 358, audience: "all" },
  { id: "veri-tarihi", name: "Burcu İnce", handle: "@burcuince", time: "11 sa", body: "Okul çevresindeki geçitleri gösteren haritaya baktım. İşaretler duruyor, son güncelleme 2023. Yol değişmiş, harita yerinde saymış.", initials: "Bİ", tone: "teal", replies: 26, reposts: 49, likes: 473, audience: "all" },
  { id: "excel-haritasi", name: "Mehmet Kaan", handle: "@mkaannot", time: "11 sa", body: "Veriyi indirmek için yine Excel açtım. En faydalı sütun dosyanın en sonunda, başlığı da 'Aciklama2'. Bunu kullanan biriyle iki dakika konuşmak istiyorum.", initials: "MK", tone: "ink", replies: 33, reposts: 37, likes: 514, audience: "following" },
  { id: "perde-sonrasi", name: "Ece Poyraz", handle: "@ecepoyraz", time: "12 sa", body: "Dünkü kısa film seçkisinden sonra kapının önünde kalan sohbet, filmlerden uzundu. Herkes aynı sahneyi başka yerinden yakalamış. Güzel tarafı bu galiba.", initials: "EP", tone: "coral", replies: 21, reposts: 14, likes: 286, audience: "all", repostedBy: "Ediz Yılmaz", repostQuote: "Filmi değil, çıkıştaki konuşmayı düşünerek eve döndüm." },
  { id: "yazlik-gosterim", name: "Sarp Kılıç", handle: "@sarpkilic", time: "12 sa", body: "Yazlık gösterimde ses biraz kısıktı ama kimse kalkmadı. Arka sıradan biri replikleri fısıldadı, ön sıradaki çocuk da perdeye el salladı. Fazla resmi olmayan sinema iyi geliyor.", initials: "SK", tone: "violet", replies: 15, reposts: 9, likes: 231, audience: "following" },
  { id: "kutuphane-masa", name: "Zeynep Kır", handle: "@zeynepkir", time: "13 sa", body: "Kütüphane 22.00'ye kadar açıkmış. Asıl rahatlatan şey saat değil, çıkınca eve yürünecek yolun aydınlık olması. İçerideki sessizlik tek başına yetmiyor.", initials: "ZK", tone: "gold", replies: 29, reposts: 43, likes: 497, audience: "all" },
  { id: "gece-durak", name: "Onur Çelik", handle: "@onurcelik", time: "13 sa", body: "Gece otobüsünü beklerken durağın ekranı üç kez 'yaklaşıyor' dedi. Sonra otobüs başka sokaktan geçti. Uygulama değil, gecenin kendisi şaka yapıyor sandım.", initials: "OÇ", tone: "teal", replies: 18, reposts: 27, likes: 362, audience: "all" },
  { id: "rampa-sorusu", name: "Gökçe Işık", handle: "@gokceisik", time: "14 sa", body: "Yeni rampanın başında park eden iki motosiklet vardı. Rampa yapılmış, yol yine yok. Bazen sorun çizimde değil, o çizimin iki metre önünde başlıyor.", initials: "Gİ", tone: "coral", replies: 42, reposts: 66, likes: 708, audience: "all" },
  { id: "puset-denemesi", name: "Funda Acar", handle: "@fundaacar", time: "14 sa", body: "Pusetle kısa yolu denedim; kısa olan merdivenli çıktı. Haritada rota var, eğim yok, kaldırım genişliği yok. Bir gün biri bunu gerçekten yürüyerek güncellesin.", initials: "FA", tone: "violet", replies: 37, reposts: 54, likes: 621, audience: "following" },
  { id: "sabah-iskele", name: "Mert Ege", handle: "@mertege", time: "15 sa", body: "İskelede ilk vapuru beklerken şehir henüz karar vermemiş gibi oluyor. Simit poşeti, turnike sesi, uzaktan bir martı. Sonra bir anda herkes aynı anda yetişmeye çalışıyor.", initials: "ME", tone: "ink", replies: 13, reposts: 11, likes: 247, audience: "all" },
  { id: "vapur-hatti", name: "Ayla Korkmaz", handle: "@aylakorkmaz", time: "15 sa", body: "Vapur hattının saatleri camda asılı, uygulamada başka yazıyor. İkisinden biri doğruysa bile insan hangisine güveneceğini bilemiyor.", initials: "AK", tone: "gold", replies: 24, reposts: 31, likes: 405, audience: "all" },
  { id: "mahalle-panosu", name: "Sibel Tan", handle: "@sibeltan", time: "16 sa", body: "Köşedeki mahalle panosunda kayıp kedi ilanı, çocuk atölyesi ve fazla domates notu yan yanaydı. Apartman grubundan daha çok haber aldım, biraz da daha az gerildim.", initials: "ST", tone: "teal", replies: 11, reposts: 8, likes: 196, audience: "following" },
  { id: "komsu-sesi", name: "Arda Bilgin", handle: "@ardabilgin", time: "16 sa", body: "Akşam mahalle panosuna biri 'matkabı ödünç alabilir miyim?' diye yazmış. Altına üç kişi telefon bırakmış. İnternet bazen en iyi işini duvarda asılı bir A4 kadar sade yapıyor.", initials: "AB", tone: "coral", replies: 20, reposts: 16, likes: 278, audience: "all" },
];
...
function countThreadReplies(nodes: ThreadReplyNode[]): number {
  return nodes.reduce((count, node) => count + 1 + countThreadReplies(node.children), 0);
}
...
function PostCard({ post, allPosts, actions, pinned, following, points, onAction, onReply, onDetail, onProfile, onShare, onMore, onMedia, onFollow, threaded = false, hasChildReplies = false, showCommentPreviews = true }: FeedProps & { post: Post; threaded?: boolean; hasChildReplies?: boolean }) {
  ...
  const interactions = { replies: visibleReplyCount(post, allPosts, !threaded), reposts: post.reposts + Number(reposted), likes: post.likes + Number(liked) };
  ...
  return <article className={"post " + (showThreadRail ? "has-thread " : "") + (showCommentPreviewsForPost ? "has-comment-previews " : "") + (showFeedRail ? "has-feed-rail" : "")} onClick={() => onDetail(post)}>
    ...
    <div className="post-content">
      ...
      {post.body && <button className="post-body" onClick={(event) => runExclusive(event, () => onDetail(post))} aria-label={post.body + " — gönderi ayrıntılarını aç"}>{post.body}</button>}
      ...
      {showCommentPreviewsForPost && <OrganicComments post={post} onDetail={onDetail} />}
    </div>
  </article>;
}
...
function Explore(props: Omit<FeedProps, "posts" | "morePost"> & { search: string; setSearch: (value: string) => void; posts: Post[] }) { const visible = postsForTrend(props.posts, props.search); return <div className="explore-view"><div className="explore-search"><span aria-hidden="true"><UiIcon name="search" /></span><input value={props.search} onChange={(event) => props.setSearch(event.target.value)} placeholder="Kişi ya da konu ara" aria-label="Kişi ya da konu ara" /></div><div className="mobile-follow-suggestions"><WhoToFollow following={props.following} onFollow={props.onFollow} /></div>{props.search ? <><h2>Arama sonuçları</h2>{visible.length ? <div className="search-results"><Feed {...props} posts={visible} morePost={null} /></div> : <EmptyState title="Bir sonuç bulamadık" copy="Başka bir kelimeyle aramayı dene." />}</> : <><h2>Bugün konuşulanlar</h2><TrendPanel full onSelect={props.setSearch} /><section className="editor-pick"><p>Editörün seçimi</p><button onClick={() => props.onDetail(seedPosts[0])}><span className="editor-ripple"><i /><i /></span><span><b>Şehir nasıl serin kalır?</b><small>İklim ve kamusal alan üzerine açık konuşma</small></span><em aria-hidden="true"><UiIcon name="arrow-right" /></em></button></section></>}</div>; }
...
function Messages({ messages, setMessages, target }: { messages: Message[]; setMessages: (next: Message[]) => void; target: Profile | null }) {
  ...
  const contacts = [
    { id: "team", name: "NSosyal Ekibi", handle: "@nsosyalekibi", initials: "YE", tone: "coral" },
    { id: "bora", name: "Bora Ekin", handle: "@boraekin", initials: "BE", tone: "violet" },
    { id: "selin", name: "Selin Uçak", handle: "@selinucak", initials: "SU", tone: "teal" },
    ...(target && !["@boraekin", "@selinucak"].includes(target.handle) ? [{ id: target.handle, name: target.name, handle: target.handle, initials: target.initials, tone: "ink" }] : []),
  ];
  ...
  const previewFor = (conversationId: string) => { const latest = [...messages].reverse().find((message) => message.conversationId === conversationId); return latest ? (latest.from === "me" ? "Sen: " : "") + latest.body : "Henüz mesaj yok"; };
