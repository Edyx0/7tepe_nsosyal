...
const navigation: { id: View; label: string; icon: UiIconName }[] = [
  ...
  { id: "contribute", label: "Katkı", icon: "spark" },
...
<Sidebar view={view} setView={setView} profile={profile} unread={3 - readNotifications.length} onOwnProfile={openOwnProfile} onCompose={openComposer} onSearch={openSearch} />
 ...
 <MobileNav view={view} setView={setView} unread={3 - readNotifications.length} composeHidden={composeHidden} onSearch={openSearch} onContribute={() => setView("contribute")} />
...
function MobileNav({ view, setView, unread, composeHidden, onSearch, onContribute }: { view: View; setView: (view: View) => void; unread: number; composeHidden: boolean; onSearch: () => void; onContribute: () => void }) { const openHome = () => { setView("feed"); window.scrollTo({ top: 0, behavior: "smooth" }); }; const mobileItems = navigation.filter((item) => item.id !== "contribute").slice(0, 4); return <nav className={"bottom-nav " + (composeHidden ? "is-compact" : "")} aria-label="Mobil gezinme">{mobileItems.map((item) => <button key={item.id} className={view === item.id ? "is-active" : ""} onClick={() => item.id === "feed" ? openHome() : item.id === "explore" ? onSearch() : setView(item.id)} aria-current={view === item.id ? "page" : undefined} aria-label={item.label}><UiIcon name={item.icon} />{item.id === "notifications" && unread > 0 && <b />}</button>)}<button className={"bottom-contribute " + (view === "contribute" ? "is-active" : "")} onClick={onContribute} aria-current={view === "contribute" ? "page" : undefined} aria-label="Katkını derinleştir"><UiIcon name="spark" /></button></nav>; }
...
function Feed(props: FeedProps) { return <div className="post-list">{props.posts.map((post) => { const displayPost = withLocalReplyCount(post, props.allPosts); return <PostCard key={post.id} {...props} post={displayPost} />; })}</div>; }
