...
function ThreadReplyBranch({ node, depth, allPosts, feedProps }: { node: ThreadReplyNode; depth: number; allPosts: Post[]; feedProps: FeedProps }) { return <div className="thread-reply-node" data-depth={depth}><PostCard {...feedProps} post={withLocalReplyCount(node.post, allPosts)} />{node.children.length > 0 && <div className="thread-children">{node.children.map((child) => <ThreadReplyBranch key={child.post.id} node={child} depth={depth + 1} allPosts={allPosts} feedProps={feedProps} />)}</div>}</div>; }
