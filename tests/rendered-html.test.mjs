...
test("keeps the context-summary and device-local demo behavior in product source", async () => {
  ...
  assert.match(page, /Bu konuşmayı bir adım ileri taşı/);
  assert.match(page, /Yaklaşımını seç/);
  ...
  assert.match(css, /\.inline-intents > div \{[\s\S]*backdrop-filter: blur\(14px\)/);
  assert.match(css, /\.inline-intents button\.is-selected/);
